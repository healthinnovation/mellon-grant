...
cp  <- st_read("Iquitos/resources/centros_poblados.gpkg") %>% 
  dplyr::select(IDCCPP_17,NOMCCPP_17)
# cp_ee <- cp |> sf_as_ee()
lima <- st_read("Iquitos/resources/iquitos_polygon.gpkg") %>%
  st_bbox() %>%
  st_as_sfc() %>%
  sf_as_ee()
travel_time <- function(x){
  ...
  distance <- distance$clip(lima)
  # dowloading raster -----------------------------------------------
  namecp <- cp[x,2] |> st_drop_geometry() |> pull() %>% gsub(" ","",.)
  raster_traveltime <- ee_as_raster(
    distance,
    dsn = sprintf("/home/am/Descargas/cp_%s%s",namecp,".tif"),
    scale = 500
  ) |> 
    st_as_stars()
  
  newdata <- raster_traveltime |> 
    mutate(
      clases = case_when(
        cumulative_cost > 0 & cumulative_cost <= 30 ~ 1,
        cumulative_cost > 30 & cumulative_cost <= 60 ~ 2,
        cumulative_cost > 60 & cumulative_cost <= 90 ~ 3,
        cumulative_cost > 90 & cumulative_cost <= 120 ~ 4,
        cumulative_cost > 120 & cumulative_cost <= max(raster_traveltime$cumulative_cost) ~ 5
      )
    )
  catchment <- newdata |> st_as_sf() |> filter(clases %in% c(1,2)) |> group_by(clases) |> summarise()
  write_sf(catchment,sprintf("/home/am/Descargas/gpkg-iquitos/%s%s",namecp,".gpkg"))
}

lista_tt <- lapply(207:217,travel_time)
