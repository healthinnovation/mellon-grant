...
dir.create(here::here("Iquitos/pbf"))
...
cp <- st_read("DataBase/centro_poblado_INEI.gpkg") |> 
  filter(DEPARTAMEN == "LORETO" & DISTRITO %in% c("IQUITOS","PUNCHANA","SAN JUAN BAUTISTA","BELEN")) |> 
  rename(CODCP = IDCCPP_17)
# cs - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
cs <- st_read("DataBase/personal_salud.gpkg") |> 
  filter(dep == "LORETO" & dist %in% c("IQUITOS","PUNCHANA","SAN JUAN BAUTISTA","BELEN")) |> 
...
ggsave(
  filename = "Iquitos/png/iquitos_modelos.png",
  plot = last_plot(),
  width = 11,
  height = 5,
  bg = "white"
)
write_csv(access_modelos |> st_drop_geometry(),"loreto_access_models.csv")
